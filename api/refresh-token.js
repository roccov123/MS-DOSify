...
export default async function handler(req, res) {
  ...
  if (!APP_PASSWORD || !CLIENT_ID || !CLIENT_SECRET || !REFRESH_TOKEN) {
    return res.status(500).json({
      error: "Missing environment variables. Check Vercel project settings.",
    });
  }

  // The front end sends whatever the admin typed in the x-app-password
  // header. This is the ONLY thing standing between a random visitor and
  // full control of your Spotify account, so check it before doing
  // anything else.
  const suppliedPassword = req.headers["x-app-password"];
  if (suppliedPassword !== APP_PASSWORD) {
